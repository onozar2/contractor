...
(function () {
  ...
  function answerHtml(result, extraPills, photoHtml) {
    ...
    html += '<div class="card" style="margin-bottom:0.9rem" id="kIllu">' +
      ...
      '<span class="muted" style="font-size:0.74rem;margin-left:0.5rem">API generation costs ~8¢ for the 2-visual set with billing; the Gemini button uses your subscription instead - paste (Ctrl+V) when it opens</span></div>';
  ...
  function render(container) {
    ...
    out.addEventListener("click", function (event) {
      ...
      card.innerHTML = '<span class="muted">Drawing 2 visuals… ~20-60s</span>';
      APP.fetchJSON("/api/knowledge/illustrate", {
        ...
        var tail = '<div class="muted" style="font-size:0.72rem;margin-top:0.4rem">generated by ' + APP.esc(r.model || "Google image model") + ' — illustrative, not engineering guidance</div>';
        if (r.images && r.images.length) {
          card.innerHTML =
            '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:0.6rem">' +
              r.images.map(function (im) {
                var legendHtml = "";
                if (im.legend && im.legend.length) {
                  legendHtml = '<ol class="muted" style="font-size:0.72rem;margin:0.25rem 0 0 1.2rem;padding:0">' +
                    im.legend.map(function (item) { return '<li>' + APP.esc(item) + '</li>'; }).join("") +
                    '</ol>';
                }
                return '<div><img src="' + APP.esc(im.imageUrl) + '" style="max-width:100%;border-radius:8px" />' +
                  (im.caption ? '<div class="muted" style="font-size:0.72rem;margin-top:0.2rem">' + APP.esc(im.caption) + '</div>' : "") +
                  legendHtml +
                  '</div>';
              }).join("") +
            "</div>" + tail;
        } else if (r.imageUrl) {
          card.innerHTML = '<img src="' + APP.esc(r.imageUrl) + '" style="max-width:560px;width:100%;border-radius:8px" />' + tail;
        } else {
          card.innerHTML = '<span class="muted">' + APP.esc(r.error || "Generation failed") + '</span>';
        }
